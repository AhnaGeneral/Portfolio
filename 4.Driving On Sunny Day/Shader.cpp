...
D3D12_SHADER_BYTECODE CIlluminatedShader::CreateVertexShader()
{
	...
	(L"Shaders.hlsl", "VSLighting", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CIlluminatedShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile
	(L"Shaders.hlsl", "PSLighting", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void CTerrainShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	d3dPipelineStateDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH; //이거 패치로 바뀌어 함
...
void CBillboardShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	for (int z = 2; z <= 30'000; z++)
	{
			nFlowerObjects++;
	}

	int m_nObjects = m_nInstances = nFlowerObjects;
	...
	pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/flower_white.dds", 0);
	...
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, pBillboardTexture, 6, false);
...
D3D12_INPUT_LAYOUT_DESC CBillboardShader::CreateInputLayout()
{
	...
	//마지막 매개변수 1 : InstanceDataStepRate = 인스턴스 데이터가 몇개의 인스턴스에 반복되는 가를 정의한다.
...
D3D12_INPUT_LAYOUT_DESC CStartShader::CreateInputLayout()
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_SHADER_BYTECODE CStartShader::CreateVertexShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSStartboard", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CStartShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSStartboard", "ps_5_1", &m_pd3dPixelShaderBlob));
}

void CStartShader::CreateShader(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, ID3D12RootSignature * pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState *[m_nPipelineStates];
